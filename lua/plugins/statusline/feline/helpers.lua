...
function M.conditional.git_changed()
    ...
    return git_status
        and (git_status.added or 0) + (git_status.removed or 0) + (git_status.changed or 0) > 0
end
...
function M.conditional.bar_width(n)
    return function()
        return (vim.opt.laststatus:get() == 3 and vim.opt.columns:get() or vim.fn.winwidth(0))
            > (n or 80)
    end
end
...
function M.active_buffer_size()
    ...
    local buffer_content = vim.fn.join(vim.fn.getbufline(current_buffer, 1, buffer_lines))
